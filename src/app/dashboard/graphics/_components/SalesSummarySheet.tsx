...
export default function SalesSummarySheet({
  open,
  setOpen,
  salesPeriod,
  setSalesPeriod,
      ...
      <SheetContent className="px-6">
        ...
        <div>
          <span className="text-sm text-gray-600">
            Exibindo {salesData.length} resultados.
          </span>
          <span className="text-sm text-gray-600">
            Total de vendas:{" "}
            <span className="font-medium text-green-600">
              {formatCurrency(
                salesData.reduce((acc, s) => acc + s.totalSales, 0)
              )}
            </span>
          </span>
          <span className="text-sm text-gray-600">
            ...
            <span className="font-medium text-green-600">
              {formatCurrency(
                salesData.reduce((acc, s) => acc + s.profit, 0)
              )}
            </span>
          </span>
        </div>
