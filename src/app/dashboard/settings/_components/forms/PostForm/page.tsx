...
import { Dialog, DialogTrigger, DialogContent, DialogHeader, DialogTitle, DialogDescription, DialogFooter, DialogClose } from "@/_components/ui/dialog";
        ...
        <CardContent className="p-4">
          <form onSubmit={handleSubmit} className="grid gap-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <Input name="weight" type="number" step="0.01" value={form.weight} onChange={handleChange} placeholder="Peso (Kg)" />
              <Input name="price" type="number" step="0.01" value={form.price} onChange={handleChange} placeholder="Preço Base (R$/Kg)" />
            </div>
            ...
            <CardContent className="flex justify-between items-center p-4">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {post.category?.name || "Sem categoria"} • {post.weight.toFixed(2)}kg • R${post.price.toFixed(2)} → Venda R${(post.sellPrice ?? post.price * (1 + MARGIN_PERCENTAGE)).toFixed(2)}
                </p>
              </div>
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={() => handleEdit(post)}>Editar</Button>
                <Dialog>
                  <DialogTrigger asChild><Button size="sm" variant="destructive">Excluir</Button></DialogTrigger>
                  <DialogContent>
                    <DialogHeader>
                      <DialogTitle>Excluir</DialogTitle>
                      <DialogDescription>Excluir {post.title}?</DialogDescription>
                    </DialogHeader>
                    <DialogFooter>
                      <DialogClose asChild><Button size="sm" variant="destructive" onClick={() => handleDelete(post.id)}>Confirmar</Button></DialogClose>
                    </DialogFooter>
                  </DialogContent>
                </Dialog>
              </div>
